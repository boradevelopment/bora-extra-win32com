...
HRESULT CreateBappIconHandlerInstance(REFIID riid, void** ppv) {
    *ppv = nullptr;
    BappIconHandler* handler = new (std::nothrow) BappIconHandler();
...
STDAPI  DllRegisterServer() {
    ...
    RegisterKeyAndValue(HKEY_CLASSES_ROOT, L"BAppFile\\shell\\open\\command", nullptr, L"\"C:\\Projects\\bora\\runtime\\builds\\debug\\BORA.exe\" \"%1\" %*");
    return S_OK;
}
...
STDAPI   DllUnregisterServer() {
    ...
    SHDeleteKeyW(HKEY_CLASSES_ROOT, L"BAppFile\\shell\\open\\command");
    SHDeleteKeyW(HKEY_CLASSES_ROOT, L"BappFile\\shellex\\IconHandler");
